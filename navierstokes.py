...
class IncompressibleNavierStokesSolver:
    ...
    def __init__(self, mesh, timestep, p=2, density_viscosity_order=1, kinematic=False) -> None:
        ...
        self.mesh = mesh
        self.tree = dolfinx.geometry.bb_tree(mesh, 2)
        # Taylor-Hood Elements
        self.pressure_space = fem.FunctionSpace(mesh, ("CG", p - 1))
        self.velocity_space = fem.VectorFunctionSpace(mesh, ("CG", p))
        ...
        u_t = ufl.TrialFunction(self.velocity_space)
        v = ufl.TestFunction(self.velocity_space)
        p_t = ufl.TrialFunction(self.pressure_space)
        q = ufl.TestFunction(self.pressure_space)
        # Solution fields
        self.u = fem.Function(self.velocity_space)
    ...
    def set_velocity_bc(self, geometry_fn, value):
        ...
        will be placed.
        fn: The function (or constant) to be imposed."""
        dofs = fem.locate_dofs_geometrical(self.velocity_space, geometry_fn)
        self.u_bcs.append(fem.dirichletbc(dolfinx.default_scalar_type(value), dofs, self.velocity_space))
    ...
    def set_x_velocity(self, geometry_fn, value):
        ...
        boundary_dofs_x = fem.locate_dofs_topological(self.velocity_space.sub(0), self.mesh.topology.dim - 1, boundary_facets)
        self.u_bcs.append(fem.dirichletbc(dolfinx.default_scalar_type(value), boundary_dofs_x, self.velocity_space.sub(0)))
    ...
    def set_y_velocity(self, geometry_fn, value):
        ...
        fn: The function (or constant) to be imposed."""
        boundary_facets = locate_entities_boundary(self.mesh, self.mesh.topology.dim - 1, geometry_fn) 
        boundary_dofs_x = fem.locate_dofs_topological(self.velocity_space.sub(1), self.mesh.topology.dim - 1, boundary_facets)
        self.u_bcs.append(fem.dirichletbc(dolfinx.default_scalar_type(value), boundary_dofs_x, self.velocity_space.sub(1)))
    ...
    def eval_velocity(self, x, y, force=False):
        _, _, u, v = fem_vector_func_at_given_points(self.u, self.mesh, self.tree, x, y, forse_eval=force)
        return u, v
